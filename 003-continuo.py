...
class Star:
    def __init__(self):
        ...
        self.speed = np.random.rand() * 0.5 + 0.1  # Random speed for each star
...
for frame in range(total_frames):
    ...
    # Gradually add new stars
    if len(stars) < num_stars and np.random.rand() < 0.05:  # Adjust probability for smoother addition
        stars.append(Star())
    ...
    for star in stars:
        ...
        if star.distance > 1000:
            ...
            star.speed = np.random.rand() * 0.5 + 0.1  # Recalculate speed for the new star
